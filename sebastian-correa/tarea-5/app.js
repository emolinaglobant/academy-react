...
function fisherYates(array) {
  return array
    .map((value) => ({ value, randomIndex: Math.random() }))
    .sort((current, random) => current.randomIndex - random.randomIndex)
    .map(({ value }) => value);
}
...
function initializeDOM() {
  playerOrder.forEach((row, posX) => {
    row.forEach((column, posY) => {
      let element = document.querySelector(`img[data-position='${posX}${posY}']`);
      element.dataset.value = playerOrder[posX][posY];
    });
  });
}
